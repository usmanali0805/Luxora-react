    ...
    {data && (
      <div className="flex flex-col gap-5 py-4 sm:py-10">
        ...
        <div className="w-full h-[4vh] flex items-center px-6">
          ...
          <span className="text-[12px] sm:text-[15px] text-blue-600">Category</span>
          <ChevronRight  className="w-[12px] h-[12px] sm:w-[15px] sm:h-[15px] text-blue-600" />
          <Link to={`/search/${data.category}`}>
            <span className="text-[12px] sm:text-[15px] text-blue-600 cursor-pointer">{data.category}</span>
          </Link>
        </div>
            ...
            <div className="sm:flex gap-2 w-full">
              ...
              <div className="sm:w-[88%] w-full px-2 sm:px-5 flex flex-col gap-2 sm:gap-4 py-5 sm:py-10">
                ...
                <div className="flex justify-between">
                  <span className="text-[12px] sm:text-[15px]">Rating: {data.rating}</span>
                  <Stack spacing={1}>
                    <Rating
                ...
                <div className="text-regular flex items-center gap-2">
                  <span className="text-slate-500 sm:text-[15px] text-[12px]">Brand:</span>
                  <span className="text-blue-600 sm:text-[15px] text-[12px] font-semibold">{data.brand}</span>
                </div>
  
                {/* Quantity */}
                <div className="quantity flex items-center gap-2">
                  <span className="text-slate-500 sm:text-[15px] text-[12px]">Quantity:</span>
                ...
                <div className="sm:text-[15px] text-[12px] text-slate-500">Stock: {data.stock}</div>
                ...
                <div className="sm:text-[15px] text-[10px]">{data.description}</div>
  
                <ul className="list-disc px-3 text-sm">
                  <li className="sm:text-[15px] text-[10px]">sku: {data.sku}</li>
                  <li className="sm:text-[15px] text-[10px]">{data.shippingInformation}</li>
                  <li className="sm:text-[15px] text-[10px]" >{data.returnPolicy}</li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div className="reviews w-full px-5 py-5 sm:px-10 sm:py-10 flex flex-col border-t border-slate-300">
          ...
          {data.reviews.length > 0 ? (
            data.reviews.map((review, index) => (
              <div key={index} className="flex flex-col px-2 sm:px-3 gap-1 py-2 sm:py-6 w-full">
                ...
                <div className="sm:text-xl text-[15px] font-semibold">{review.comment}</div>
              </div>
            ))
          ) : (
